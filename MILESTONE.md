...
- 主动上报帧（NODE_JOIN）处理：已确认 LM960B 无主动上报，相关死代码已移除（2026-04-04）
...
- ~~模式切换上报帧格式~~ ✓ 已确认：LM960B 无主动上报，模式变化靠轮询 `@0.<mac>/3/20` 感知（2026-04-04）
- ~~NODE_JOIN 上报帧格式~~ ✓ 已确认：LM960B 无主动上报，节点加入靠广播发现轮询感知（2026-04-04）
